...
- **HTTP caching**: Module declarations are cached together in the HTTP cache with the enclosing module. There is no way for the browser to request just the declarations it is missing. By contrast, [resource bundle loading](https://github.com/WICG/bundle-preloading/blob/main/subresource-loading-client.md) divides resources into chunks, and only the chunks which are not present in cache are loaded.
